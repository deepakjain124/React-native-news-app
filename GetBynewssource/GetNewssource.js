...
import Allcards from "./Allcards";
  const GetByNewsSource = () => {
    const navigation=useNavigation()
    const [isVisible, setIsVisible] = useState(false);
    const[search,setsearch]=useState("")
    const [APIDATA, setAPIDATA] = useState([]);
    const[filter,setfilter]=useState([])
    // console.log(JSON.stringify(APIDATA),"APIDATA");
    ...
    const getlatest = async () => {
      try {
        const response = await fetch(
          "https://newsdata.io/api/1/sources?apikey=pub_93229c984214128774aa6852957de6f8e804"
        );
    ...
    return (
      ...
      <View style={styles.container}>
        ...
        <Text style={{fontSize:20,color:"#fff",backgroundColor:"black",padding:5,borderRadius:50}}>Sign Out</Text>
        </TouchableOpacity>
        </View>
        ...
        <Allcards searchdata={searchdata} filter={filter} loader={loader} APIDATA={APIDATA} />
  ...
  export default GetByNewsSource;
